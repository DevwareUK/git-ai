...
export const PRS_CODEX_SKILLS: ManagedCodexSkill[] = [
  {
    ...
    body: [
      ...
      `- \`/prs pr <number>\`: run \`${renderPrReadyToolCommand()}\`; prepare the actual PR head branch in the current repository checkout used by the user's normal local runtime, fetch and merge the latest PR base branch, summarize \`prContext\` signals from GitHub, and stop with the next sensible action so the user can browse the app quickly. If the PR head branch is locked by a clean prs worktree, the tool may remove that clean worktree and then check out the actual PR branch in the current checkout. If that worktree has uncommitted changes, stop and report the blocker.`,
  ...
  },
  {
    ...
    body: [
      ...
      `For \`/prs:pr <number>\`, run \`${renderPrReadyToolCommand()}\`; prepare the actual PR head branch in the current repository checkout used by the user's local runtime, fetch and merge the latest PR base branch, summarize \`prContext\` GitHub signals, and stop once the app is ready to browse or a blocker is clear.`,
      ...
      "After readiness, offer the next sensible step: browse/functional test first, then inspect failed or pending checks, managed AI test suggestions, and actionable review comments before any explicit fix/review command.",
    ].join("\n"),
  },
  {
