...
export const PRS_CODEX_SKILLS: ManagedCodexSkill[] = [
  {
    ...
    body: [
      ...
      "- `/prs pr <number> fix-comments`: run `prs tool pr fix-comments <number> --json`, read the returned `promptFilePath` and `snapshotFilePath`, then continue the selected fixes in this Codex session. Do not run a command that launches nested Codex.",
      "- `/prs pr <number> fix-failing-tests`: run `prs tool pr fix-failing-tests <number> --json`, read the returned `promptFilePath` and `snapshotFilePath`, then continue the failing-test fix in this Codex session. Do not run a command that launches nested Codex.",
      "- `/prs pr <number> fix-tests`: run `prs tool pr fix-tests <number> --json`, read the returned `promptFilePath` and `snapshotFilePath`, then continue the selected test work in this Codex session. Do not run a command that launches nested Codex.",
